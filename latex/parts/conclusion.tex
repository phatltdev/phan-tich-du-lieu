...
Kết quả trên phù hợp với phân tích dữ liệu: bộ Breast Cancer có đặc trưng liên tục tách lớp rõ nên phân loại tốt; CDC Diabetes có tín hiệu yếu hơn nên cần xử lý imbalance và feature selection; Appliances Energy phụ thuộc trend, seasonality và hành vi sử dụng nên mô hình tức thời chưa đủ mạnh. Phần clustering đã bổ sung thực nghiệm K-Means, Hierarchical và DBSCAN trên ba đại diện A/B/C, cho thấy HAM10000 có cấu trúc cụm theo độ sáng/tương phản rõ nhất, Appliances Energy phân hóa theo nhiều trạng thái tiêu thụ, còn Heart Disease tách được nhóm rủi ro cao hơn qua tuổi, cholesterol, \texttt{thalach} và \texttt{oldpeak}.

\section{Hạn chế}

Báo cáo vẫn còn một số hạn chế. Thứ nhất, artifact phân tích sâu hiện tập trung nhiều vào Heart Disease, Appliances Energy, HAM10000 và các baseline nhóm A; HAR, MHEALTH và Reuters-21578 cần được mở rộng thêm bảng thống kê, hình và mô hình. Thứ hai, clustering đã có bảng kết quả chính thức nhưng mới chạy trên ba đại diện A1/B1/C1, chưa mở rộng đủ cho toàn bộ 8 dataset. Thứ ba, đối sánh với nghiên cứu liên quan mới ở mức định tính; cần bổ sung trích dẫn chuẩn IEEE/APA và số liệu từ bài báo để so sánh định lượng. Thứ tư, phạm vi báo cáo hiện tại còn gọn, nên cần tiếp tục phát triển phụ lục và phân tích chi tiết để mở rộng độ sâu của nghiên cứu.
...
\begin{enumerate}
    ...
    \item Mở rộng clustering từ ba đại diện A1/B1/C1 sang toàn bộ nhóm A, B, C; bổ sung Elbow plot và bảng diễn giải cụm chi tiết hơn cho từng dataset.
    ...
    \item Chuẩn hóa tài liệu tham khảo theo IEEE hoặc APA, đặc biệt các nghiên cứu dùng cùng dataset để đối sánh Chương 5.
...
\begin{itemize}
    ...
    \item Script phân tích và học máy: \texttt{src/analyze\_heart\_disease.py}, \texttt{src/analyze\_tabular\_health.py}, \texttt{src/analyze\_appliances\_energy.py}, \texttt{src/analyze\_ham10000.py}, \texttt{src/run\_ml\_baselines.py}, \texttt{src/create\_adjusted\_report\_artifacts.py}.
    \item Bảng kết quả: \texttt{outputs/tables/}.
    \item Hình minh họa: \texttt{outputs/figures/}.
    \item Log thực nghiệm: \texttt{outputs/logs/}.
    \item Workbook Excel đối chiếu: \texttt{excel/excel\_visualization\_comparison\_native.xlsx}; sheet \texttt{Report Chart Index} liệt kê sheet chứa từng biểu đồ Excel native.
\end{itemize}
