...
Có giá trị ngoại lệ thực tế & Tồn tại một số bệnh nhân cholesterol rất cao (tới 564 mg/dL), phù hợp minh họa phát hiện và xử lý outlier \\
...
\textbf{Hạn chế.} Mean nhạy với outlier và phân phối lệch mạnh: chỉ vài giá trị cực trị cũng đủ kéo lệch kết quả. Mean không dùng cho biến định tính; khi dữ liệu lệch cần đọc kèm median, IQR và boxplot.
...
\textbf{Nhận xét kết quả Mean.} Bảng \ref{tab:chapter1_mean_example} cho biết ``giá trị đại diện'' của từng đặc tính: tuổi trung bình 54.44 (người trung niên), cholesterol 24.69 mg/dL (vượt ngưỡng khuyến nghị <200). Điểm quan trọng: mean của \texttt{chol} giảm từ 246.69 xuống 245.58 sau khi cắt outlier -- chỉ 5/303 bệnh nhân có cholesterol rất cao cũng đủ kéo mean lên hơn 1 đơn vị. \textbf{Mean nhạy với outlier}, nên khi dữ liệu có giá trị cực trị cần đọc kèm median và IQR.
...
\textbf{Nhận xét kết quả Median.} Khoảng cách giữa mean và median là dấu hiệu độ lệch phân phối. Với \texttt{chol}, mean (246.69) $>$ median (241.00) $\Rightarrow$ \textit{lệch phải}; với \texttt{age}, hai số đo gần trùng nhau $\Rightarrow$ phân phối cân đối. Tính bền vững của median thể hiện rõ: \textbf{median của \texttt{chol} giữ nguyên 241.00 cả trước và sau khi cắt outlier}, trong khi mean thay đổi.
...
\textbf{Nhận xét kết quả Trimmed Mean.} Quy tắc đọc: \textbf{khoảng cách mean--trimmed mean càng lớn thì biến càng bị outlier ảnh hưởng}. Với \texttt{chol}, mean (246.69) $>$ trimmed mean (243.97) $>$ median (241.00) $\Rightarrow$ vài giá trị rất cao kéo mean lên; sau khi cắt 10\% mỗi đầu, mean tiệm cận median. Với \texttt{age}, ba số đo gần trùng nhau $\Rightarrow$ phân phối cân đối, không có outlier đáng kể.
...
\textbf{Hạn chế.} Vì bình phương khoảng cách, phương sai và độ lệch chuẩn rất nhạy với outlier. Cùng một giá trị độ lệch chuẩn có thể tương ứng nhiều hình dạng phân phối khác nhau, nên cần đọc kèm histogram/boxplot.
...
\textbf{Hạn chế.} Nếu mean gần 0 hoặc âm, CV dễ gây hiểu sai vì mẫu số quá nhỏ hoặc đổi dấu. CV cao có thể do outlier chứ không phải biến động tự nhiên; không phù hợp cho biến đã chuẩn hóa quanh 0.
...
\begin{itemize}[leftmargin=1.6em,label=\textbullet]
 ...
 \item \texttt{Lower}, \texttt{Upper}: ngưỡng dưới và trên để phát hiện outlier.
\end{itemize}

Giá trị nhỏ hơn \texttt{Lower} hoặc lớn hơn \texttt{Upper} được xem là ứng viên outlier.
...
\textbf{Hạn chế.} Vì vẫn tham chiếu đến mean (bản thân mean đã bị outlier kéo lệch), $D_{\text{mean}}$ chỉ giảm bớt chứ không triệt tiêu ảnh hưởng của cực trị; khi cần độ bền vững cao hơn, nên dùng median-MAD ở mục kế tiếp.
...
\textbf{Hạn chế.} Hiệu quả thống kê của MAD chỉ khoảng 37\%, thấp hơn standard deviation; tốt cho phát hiện outlier nhưng khi dữ liệu đã sạch, standard deviation hiệu quả hơn. Với mẫu nhỏ hoặc dữ liệu rời rạc ít giá trị, MAD kém sắc nét.
...
\textbf{Hạn chế.} Vì phụ thuộc vào mean và độ lệch chuẩn, z-score nhạy với outlier và kém phù hợp khi phân phối lệch mạnh. Ngưỡng $|z|>3$ chỉ là quy tắc kinh nghiệm, không thay thế kiểm tra bằng kiến thức miền.
...
Đọc Bảng \ref{tab:chapter1_histogram_calc} là đã ``đọc'' được histogram: cột ở khoảng $[50,100)$ cao nhất (5 mẫu), cột ở $[200,300]$ lùn nhất (1 mẫu), còn khoảng giữa không có mẫu nào. Khi vẽ lên trục tọa độ với trục hoành là giá trị và trục tung là tần suất, các con số này trở thành chiều cao của các cột. Cột log1p ở bảng trên được tính bằng $\log(1+x)$, ví dụ $\log(1+60)=4.11$, chuẩn bị cho việc so sánh phân phối trước--sau biến đổi log ở Chương~3.
...
\caption[Minh họa histogram Excel]{Cùng phân phối raw của \texttt{Appliances} nhưng vẽ bằng Excel. Trục hoành (0--1100) khớp với phiên bản Python ở Hình~\ref{fig:chapter1_histogram_raw}; trục tung được đặt theo mẫu dữ liệu trong workbook Excel (1000 mẫu) nên có thang nhỏ hơn.}
\label{fig:chapter1_histogram_example_excel}
\end{figure}

\textbf{Nhận xét và lý do chọn.} Histogram được chọn vì \texttt{Appliances} là biến định lượng liên tục và câu hỏi đặt ra là ``giá trị tiêu thụ điện phân bố thế nào?''. Hình~\ref{fig:chapter1_histogram_raw} cho thấy phân phối gốc \textbf{lệch phải mạnh}: phần lớn các khung giờ tiêu thụ ít điện (cột cao ở bên trái), trong khi một số ít khung giờ tiêu thụ rất nhiều (đuôi dài kéo sang phải). So sánh Python (Hình~\ref{fig:chapter1_histogram_raw}) và Excel (Hình~\ref{fig:chapter1_histogram_example_excel}) cho thấy hai công cụ cho ra cùng hình dạng lệch phải, tức là kết luận không phụ thuộc công cụ: dữ liệu này không nên đưa thẳng vào mô hình hồi quy mà cần biến đổi log hoặc xử lý outlier trước. Python phù hợp khi cần tự động hóa và tái lập; Excel phù hợp khi cần kiểm tra nhanh bằng bảng tính.

\subsubsection{Boxplot (Biểu đồ hộp): phát hiện outliers dựa trên IQR}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{images/ch1/excel_heart_chol_boxplot_preview.png}
\caption[Phiên bản Excel boxplot cholesterol]{Phiên bản Excel boxplot: cholesterol theo nhóm \texttt{target\_binary}, vẽ theo style box-and-whisker của Excel với cùng trục tung (0--600) như Hình~\ref{fig:chapter1_boxplot_example}.}
\label{fig:chapter1_boxplot_example_excel}
\end{figure}

\textbf{So sánh Python và Excel.} Hình~\ref{fig:chapter1_boxplot_example} (Python) và Hình~\ref{fig:chapter1_boxplot_example_excel} (Excel) dùng cùng một trục tung 0--600 và cùng hai nhóm \texttt{target\_binary}, cho nên vị trí hộp, râu và các điểm outlier hoàn toàn trùng nhau: cả hai đều cho thấy nhóm có bệnh (target\_binary = 1) có median cao hơn và vài điểm cholesterol vượt 400 là ứng viên outlier. Python (matplotlib) thuận tiện khi cần tô màu hộp theo nhóm, thêm jitter hay kết hợp violin; Excel (chart Box-and-Whisker) cho phép người đọc mở workbook, thay đổi ngưỡng và quan sát hộp thay đổi theo thời gian thực.
...
\caption[Minh họa scatter plot Excel]{Minh họa scatter plot tuổi và nhịp tim tối đa trong Heart Disease bằng Excel.}
...
\caption[Minh họa bar chart Excel]{Minh họa bar chart tần suất loại đau ngực Heart Disease bằng Excel.}
...
\textbf{Pie chart 2D hay 3D -- vấn đề thẩm mỹ hay trực quan?} Một câu hỏi thường gặp là tại sao nhiều báo cáo thích dùng pie chart 3D: đó là do sở thích thẩm mỹ hay nó hỗ trợ trực quan hóa tốt hơn pie chart 2D? Câu trả lời ngắn gọn là \textbf{3D phần lớn chỉ mang yếu tố thẩm mỹ và không giúp đọc dữ liệu tốt hơn}; trong phân tích dữ liệu chuyên nghiệp, 2D vẫn là chuẩn mực và pie chart 3D \textit{ít được khuyến khích}. Lý do là góc nghiêng 3D làm \textbf{sai lệch tỷ lệ thị giác}: một lát cắt ở phía trước trông to hơn thực tế, còn lát cắt phía sau trông nhỏ hơn, khiến người đọc ước lượng sai tỷ trọng từng nhóm. Hơn nữa, với pie chart 3D, lát cắt ``nhô ra'' (exploded) càng tăng độ méo vì chiều cao của miếng bánh cộng thêm thể tích không tương xứng với giá trị. Pie chart 2D ngược lại giữ nguyên tỷ lệ góc $p_j\times360^\circ$ đã tính ở Bảng \ref{tab:chapter1_bar_calc}, nên truyền tải chính xác cơ cấu lớp. Tóm lại, nếu mục tiêu là \textit{đẹp} để trình bày nhanh thì 3D có thể chấp nhận được; nhưng nếu mục tiêu là \textit{đọc đúng số liệu} như trong EDA báo cáo này, 2D (hoặc tốt hơn là bar chart) là lựa chọn đúng đắn. Vì vậy toàn bộ pie/bar chart trong báo cáo đều dùng dạng 2D phẳng.
...
\caption[Minh họa line chart Excel]{Minh họa line chart tiêu thụ năng lượng theo ngày trong Appliances Energy bằng Excel.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.80\textwidth]{images/ch1/excel_heart_corr_heatmap_preview.png}
\caption[Phiên bản Excel correlation heatmap]{Phiên bản Excel ``heatmap'': ma trận tương quan biến Heart Disease tô màu bằng conditional formatting 3-color scale (đỏ $=-1$, trắng $=0$, xanh $=+1$), cùng thang màu RdBu\_r và cùng giá trị ô $[-1,1]$ như Hình~\ref{fig:chapter1_heatmap_example}.}
\label{fig:chapter1_heatmap_example_excel}
\end{figure}

\textbf{So sánh Python và Excel.} Hình~\ref{fig:chapter1_heatmap_example} (Python) và Hình~\ref{fig:chapter1_heatmap_example_excel} (Excel) đều dùng thang màu phân kỳ đỏ--trắng--xanh trên cùng miền giá trị $[-1, 1]$, nên mỗi ô ở hai hình có cùng màu cho cùng một hệ số $r$ (ví dụ \texttt{age}--\texttt{thalach} $= -0.39$ đều là ô xanh nhạt). Excel không có chart ``heatmap'' riêng nên thay bằng \textbf{conditional formatting 3-color scale} áp lên ma trận \texttt{=CORREL(...)}: cách này vẫn cho phép người đọc thấy nhanh cặp biến tương quan mạnh mà không cần chạy script. Python (seaborn) mạnh hơn ở việc thêm annot, cluster, mask tam giác trên; Excel mạnh hơn ở tính tương tác -- người đọc có thể thay đổi biến và xem màu cập nhật ngay.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{images/ch1/excel_appliances_ridgeline_preview.png}
\caption[Phiên bản Excel ridgeline Appliances Energy]{Phiên bản Excel ridgeline: phân phối \texttt{log1p(Appliances)} theo tháng, render bằng matplotlib nhưng dùng cùng trục hoành (2.0--7.5) và cùng danh sách tháng như Hình~\ref{fig:chapter1_ridgeline_example_inline}.}
\label{fig:chapter1_ridgeline_example_excel}
\end{figure}

\textbf{So sánh Python và Excel.} Hình~\ref{fig:chapter1_ridgeline_example_inline} (Python) và Hình~\ref{fig:chapter1_ridgeline_example_excel} (Excel) đều dùng cùng trục hoành $\log1p \in [2.0, 7.5]$ và cùng năm tháng theo thứ tự thời gian, nên hình dạng các đỉnh density hoàn toàn trùng nhau: cả hai đều cho thấy tháng 2016-01 có phân phối hẹp và lệch phải hơn, trong khi các tháng sau phân phối trải rộng hơn. Vì Excel không có chart ridgeline native, sheet ``Appliances Ridgeline'' chỉ chứa dữ liệu \texttt{month + log1p}; người đọc có thể dựng nhiều histogram theo tháng hoặc dùng pivot chart để xấp xỉ ridgeline. Python (matplotlib) là công cụ chính để render ridgeline cho báo cáo vì kiểm soát được độ trượt, độ trong suốt và thứ tự lớp.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{images/ch1/ham10000_mean_intensity_boxplot.png}
\caption[Minh họa dữ liệu cho raincloud plot]{Cơ sở thay thế bằng raincloud plot: độ sáng ảnh HAM10000 theo lớp.}
...
\centering
\includegraphics[width=0.90\textwidth]{images/ch1/heart_disease_age_thalach_scatter.png}
\caption[Minh họa dữ liệu cho hexbin plot]{Cơ sở thay thế bằng hexbin plot: tuổi và nhịp tim tối đa trong Heart Disease.}
\label{fig:chapter1_extra_chart_examples_hexbin}
\end{figure}

\textbf{Nhận xét và lý do chọn.} Ba biểu đồ mới được chọn vì mỗi biểu đồ xử lý một hạn chế cụ thể của biểu đồ cơ bản. Raincloud plot mở rộng boxplot khi cần thấy cả density và điểm dữ liệu; ridgeline plot mở rộng KDE khi cần so sánh nhiều nhóm/thời đoạn (đã minh họa cặp Python--Excel ở Hình~\ref{fig:chapter1_ridgeline_example_inline} và \ref{fig:chapter1_ridgeline_example_excel}); hexbin plot mở rộng scatter plot khi điểm quan sát quá dày. Trong báo cáo, chúng nên được dùng có chọn lọc: chỉ thêm khi câu hỏi phân tích cần mật độ phân phối, nhiều nhóm song song hoặc dữ liệu hai biến quá đông điểm. Nếu mục tiêu chỉ là mô tả đơn giản, histogram, boxplot và scatter plot vẫn là lựa chọn dễ đọc hơn.
