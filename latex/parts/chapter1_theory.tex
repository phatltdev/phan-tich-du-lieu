...
\printmyminitoc{
Chương này trình bày nền tảng thống kê mô tả và trực quan hóa dữ liệu dùng trong toàn bộ báo cáo. Các khái niệm lý thuyết được gắn với ví dụ thực tế từ bộ dữ liệu Heart Disease để làm rõ ý nghĩa phân tích trước khi áp dụng trên các nhóm dữ liệu A, B và C.
}

\section{Vai trò của thống kê mô tả trong phân tích dữ liệu}

Thống kê mô tả là bước đầu tiên để biến dữ liệu thô thành thông tin có thể đọc được. Trước khi áp dụng các thuật toán học máy, người phân tích cần biết dữ liệu có bao nhiêu mẫu, biến nào có giá trị thiếu, biến nào lệch phân phối, biến nào có biến động lớn và biến nào có khả năng phân tách nhãn mục tiêu. Trong báo cáo này, thống kê mô tả được sử dụng với ba mục đích chính:

\begin{itemize}
    \item Mô tả hình dạng và trung tâm của từng biến.
    \item Phát hiện bất thường như outlier, dữ liệu thiếu hoặc phân phối lệch.
    \item Tạo cơ sở định lượng để so sánh dữ liệu trước và sau tiền xử lý.
\end{itemize}

Với nhóm A và B, thống kê mô tả được tính trực tiếp trên các biến bảng số. Với nhóm C, dữ liệu đa phương tiện phải được số hóa thành đặc trưng định lượng trước, ví dụ độ sáng trung bình của ảnh, kích thước ảnh, độ dài văn bản, số từ hoặc tần suất từ.

\begin{table}[H]
\centering
\caption{Liên kết giữa yêu cầu phân tích và kỹ thuật sử dụng}
\label{tab:chapter1_requirement_method}
\small
\begin{tabular}{lll}
\toprule
Yêu cầu & Kỹ thuật thống kê & Trực quan hóa phù hợp \\
\midrule
Mô tả trung tâm dữ liệu & Mean, Median, Mode & Histogram \\
Mô tả độ biến động & Range, Variance, SD, CV, IQR & Boxplot \\
Phát hiện outlier & Quy tắc IQR, Z-score & Boxplot, Scatter plot \\
So sánh theo lớp & Tần suất, mean theo nhóm & Bar chart, Boxplot \\
Phân tích liên hệ biến & Correlation, covariance & Scatter plot, Heatmap \\
Phân tích chuỗi thời gian & Rolling mean, differencing & Line chart \\
Số hóa multimedia & Feature extraction & Histogram, Boxplot theo lớp \\
\bottomrule
\end{tabular}
\end{table}

Bảng \ref{tab:chapter1_requirement_method} cho thấy mỗi yêu cầu trong đề bài đều có thể ánh xạ thành một nhóm kỹ thuật cụ thể. Cách ánh xạ này giúp quá trình thực hiện không bị rời rạc: mỗi bảng và mỗi hình trong báo cáo đều phải trả lời một câu hỏi phân tích nhất định.

\section{Phân loại biến và quy trình EDA}

Trước khi tính thống kê hoặc vẽ biểu đồ cần phân loại bản chất biến. Biến \textbf{định tính} mô tả nhóm, nhãn hoặc trạng thái, ví dụ \texttt{sex}, \texttt{cp}, \texttt{Diagnosis}, \texttt{dx}, topic văn bản. Biến định tính có thể là \textit{nominal} nếu các mức không có thứ tự tự nhiên, hoặc \textit{ordinal} nếu có thứ tự như \texttt{GenHlth}, \texttt{Education}, \texttt{Income}. Biến \textbf{định lượng} biểu diễn giá trị số; biến rời rạc thường là tuổi, số ngày sức khỏe kém, số từ; biến liên tục gồm BMI, cholesterol, huyết áp, nhiệt độ, độ ẩm, năng lượng tiêu thụ, độ sáng ảnh.

\begin{table}[H]
\centering
\caption{Phân loại biến dùng trong các nhóm dữ liệu của báo cáo}
\label{tab:chapter1_variable_taxonomy}
\small
\begin{tabular}{p{0.18\textwidth}p{0.22\textwidth}p{0.26\textwidth}p{0.24\textwidth}}
\toprule
Loại biến & Ý nghĩa & Ví dụ trong báo cáo & Xử lý phù hợp \\
\midrule
Định tính nominal & Nhãn không có thứ tự & \texttt{cp}, \texttt{Diagnosis}, \texttt{dx}, topic Reuters & Tần suất, mode, bar chart, one-hot encoding \\
Định tính ordinal & Nhãn có thứ tự & \texttt{GenHlth}, \texttt{Education}, \texttt{Income} & Tần suất, median, bar chart, ordinal encoding \\
Định lượng rời rạc & Giá trị đếm được & \texttt{age}, \texttt{MentHlth}, số từ văn bản & Mean/median, histogram, boxplot \\
Định lượng liên tục & Giá trị đo lường & \texttt{chol}, \texttt{BMI}, \texttt{Appliances}, độ sáng ảnh & Thống kê đầy đủ, histogram, boxplot, scatter \\
Chuỗi thời gian & Giá trị gắn mốc thời gian/thứ tự & Appliances, tín hiệu HAR/MHEALTH & Line chart, rolling mean, interpolation, differencing \\
\bottomrule
\end{tabular}
\end{table}

Quy trình EDA trong báo cáo được triển khai theo sáu bước: hiểu nguồn dữ liệu và biến mục tiêu; phân loại biến; tính thống kê mô tả hoặc bảng tần suất; vẽ biểu đồ phù hợp; phát hiện thiếu dữ liệu, outlier và mất cân bằng lớp; tiền xử lý rồi kiểm tra lại bằng cùng hệ thống thống kê/biểu đồ. Cách làm này bảo đảm Chương 1 không chỉ là công thức rời rạc mà trở thành khung thao tác trực tiếp cho Chương 3.

\begin{table}[H]
\centering
\caption{Gợi ý chọn biểu đồ theo mục tiêu phân tích}
\label{tab:chapter1_chart_selection}
\small
\begin{tabular}{p{0.23\textwidth}p{0.27\textwidth}p{0.40\textwidth}}
\toprule
Câu hỏi phân tích & Biểu đồ chính & Lý do sử dụng \\
\midrule
Biến số phân phối ra sao? & Histogram, density plot & Nhận biết lệch trái/phải, nhiều đỉnh, khoảng giá trị phổ biến \\
Có outlier không? & Boxplot & Dựa trên quartile và IQR để nhìn điểm dị biệt \\
Hai biến số liên hệ thế nào? & Scatter plot, heatmap tương quan & Phát hiện tuyến tính, phi tuyến, cụm hoặc nhiễu \\
Biến định tính chiếm tỷ trọng nào? & Bar chart, pie chart & So sánh tần suất/tỷ lệ từng lớp \\
Chuỗi thay đổi theo thời gian ra sao? & Line chart, rolling mean & Nhìn trend, seasonality, đứt gãy và biến động \\
Hai nhóm/lớp khác nhau không? & Boxplot theo lớp, grouped bar & Đánh giá khả năng phân tách trước học máy \\
Multimedia đã số hóa ra sao? & Histogram đặc trưng, boxplot theo lớp & Kiểm tra đặc trưng ảnh/văn bản có ý nghĩa thống kê không \\
\bottomrule
\end{tabular}
\end{table}
...
Các công thức trong mục này được tổng hợp từ các tài liệu học phần trong thư mục \texttt{docments/}, đặc biệt là giáo trình \textit{Essentials of Data Science and Analytics: Statistical Tools, Machine Learning, and R-Statistical Software Overview} \cite{Sahay2021Essentials} và tài liệu nhập môn phân tích dữ liệu \textit{Data Analytics Made Easy} \cite{DeMauro2021DataAnalytics}. Theo các tài liệu này, thống kê mô tả là lớp phân tích đầu tiên của EDA: tóm tắt dữ liệu thô thành các đại lượng dễ đọc, hỗ trợ phát hiện độ lệch, outlier, mất cân bằng và tạo cơ sở chọn biểu đồ hoặc mô hình. Nhóm số đo xu hướng trung tâm trả lời câu hỏi: ``giá trị đại diện của biến nằm ở đâu?''. Tuy nhiên, không có một số đo nào luôn đúng cho mọi dữ liệu; mean, median, mode và percentiles cần được chọn theo kiểu biến, hình dạng phân phối và mục tiêu phân tích.

\subsection{Mean}

Trung bình mẫu cho biết giá trị đại diện theo nghĩa cân bằng số học của một biến định lượng. Công thức mean là công thức thống kê mô tả cơ bản được trình bày trong các tài liệu học phần \cite{Sahay2021Essentials,DeMauro2021DataAnalytics}. Với mẫu gồm $n$ quan sát $x_1, x_2, \ldots, x_n$, trung bình mẫu được tính như sau:

\begin{equation}
\bar{x} = \frac{1}{n}\sum_{i=1}^{n}x_i = \frac{x_1+x_2+\cdots+x_n}{n}
\end{equation}

Trong đó $x_i$ là giá trị quan sát thứ $i$, $n$ là số quan sát hợp lệ, $\sum$ là phép cộng toàn bộ các giá trị và $\bar{x}$ là mean. Mean có ba ý nghĩa chính. Thứ nhất, nó là điểm cân bằng số học của dữ liệu: nếu tổng giá trị được chia đều cho mọi quan sát thì mỗi quan sát nhận $\bar{x}$. Thứ hai, mean dùng toàn bộ dữ liệu nên tận dụng nhiều thông tin hơn median/mode. Thứ ba, mean là nền tảng cho nhiều phép tính sau như variance, standard deviation, z-score, regression và các thuật toán tối ưu hóa sai số bình phương.

Mean phù hợp với biến định lượng liên tục hoặc rời rạc khi phân phối tương đối cân bằng, ít outlier và cần một giá trị đại diện tổng quát. Mean đặc biệt hữu ích khi so sánh trung bình giữa các nhóm, ví dụ cholesterol trung bình của nhóm có bệnh và không bệnh tim. Trường hợp sử dụng phổ biến gồm: tính tuổi trung bình bệnh nhân, mức tiêu thụ năng lượng trung bình theo ngày, độ sáng trung bình của ảnh hoặc giá trị trung bình của đặc trưng đầu vào trước khi chuẩn hóa. \textbf{Hạn chế:} mean không phù hợp làm đại diện duy nhất khi dữ liệu lệch mạnh, có giá trị cực trị hoặc phân phối nhiều đỉnh. Mean cũng không dùng cho biến định tính và có thể bị kéo lệch chỉ bởi một vài quan sát bất thường. Trong các trường hợp đó cần đọc kèm median, IQR và boxplot.

\textbf{Ví dụ.} Bảng \ref{tab:chapter1_mean_example} minh họa mean trên nhiều đặc tính Heart Disease. Với \texttt{chol}, $\bar{x}=74748/303=246.69$; sau xử lý outlier, mean giảm còn 245.58, cho thấy mean bị ảnh hưởng bởi giá trị cholesterol rất cao.
...
\begin{tabular}{lrrr}
\toprule
Đặc tính & $n$ & Tổng giá trị & Mean \\
\midrule
\texttt{age} -- tuổi & 303 & 16498 & 54.44 \\
\texttt{trestbps} -- huyết áp nghỉ & 303 & 39949 & 131.68 \\
\texttt{chol} -- cholesterol & 303 & 74748 & 246.69 \\
\texttt{thalach} -- nhịp tim tối đa & 303 & 45343 & 149.65 \\
\texttt{oldpeak} -- ST depression & 303 & 315.0 & 1.04 \\
\bottomrule
\end{tabular}
\end{table}
...
Median là giá trị nằm ở giữa phân phối sau khi sắp xếp dữ liệu tăng dần; đây là số đo trung tâm bền vững được khuyến nghị dùng khi dữ liệu lệch hoặc có ngoại lệ \cite{Sahay2021Essentials,DeMauro2021DataAnalytics}. Gọi $x_{(1)} \le x_{(2)} \le \cdots \le x_{(n)}$ là dãy đã sắp xếp. Median được tính như sau:
...
Trong đó $x_{(j)}$ là giá trị tại vị trí thứ $j$ sau khi sắp xếp. Median có ý nghĩa là điểm chia dữ liệu thành hai nửa: khoảng 50\% quan sát nhỏ hơn hoặc bằng median và khoảng 50\% quan sát lớn hơn hoặc bằng median. Vì chỉ phụ thuộc vào vị trí sau sắp xếp chứ không phụ thuộc trực tiếp vào độ lớn của các giá trị cực trị, median là số đo trung tâm bền vững hơn mean.

Median phù hợp khi dữ liệu có outlier, phân phối lệch hoặc cần mô tả giá trị trung tâm bền vững. Median thường được dùng với thu nhập, giá nhà, chi phí y tế, cholesterol hoặc năng lượng tiêu thụ khi có vài giá trị rất lớn. Trường hợp sử dụng điển hình là báo cáo mức cholesterol đại diện khi có bệnh nhân ngoại lệ, mô tả ngày tiêu thụ điện điển hình trong chuỗi Appliances, hoặc chọn giá trị giữa của độ sáng ảnh khi ảnh có vùng quá tối/quá sáng. Trong tiền xử lý, median cũng thường được dùng để điền khuyết cho biến định lượng có outlier. \textbf{Hạn chế:} median không sử dụng độ lớn của toàn bộ quan sát, nên có thể bỏ qua một phần thông tin biến động; hai bộ dữ liệu có median giống nhau vẫn có thể có độ phân tán rất khác nhau. Median cũng kém thuận tiện hơn mean trong một số công thức tối ưu hóa và mô hình thống kê.
...
\begin{tabular}{lrrr}
\toprule
Đặc tính & $n$ & Vị trí median & Median \\
\midrule
\texttt{age} -- tuổi & 303 & 152 & 56.00 \\
\texttt{trestbps} -- huyết áp nghỉ & 303 & 152 & 130.00 \\
\texttt{chol} -- cholesterol & 303 & 152 & 241.00 \\
\texttt{thalach} -- nhịp tim tối đa & 303 & 152 & 153.00 \\
\texttt{oldpeak} -- ST depression & 303 & 152 & 0.80 \\
\bottomrule
\end{tabular}
\end{table}
...
Mode là giá trị hoặc nhóm xuất hiện nhiều nhất trong dữ liệu, thường được dùng trong mô tả biến định tính và phân phối tần suất \cite{Sahay2021Essentials,DeMauro2021DataAnalytics}:

\begin{equation}
Mode = \arg\max_v f(v), \quad f(v)=\sum_{i=1}^{n} I(x_i=v)
\end{equation}

Trong đó $v$ là một giá trị hoặc một lớp cần xét, $f(v)$ là tần suất của $v$, $I(\cdot)$ là hàm chỉ báo nhận 1 nếu điều kiện đúng và 0 nếu sai. Mode có ý nghĩa là trạng thái phổ biến nhất của dữ liệu. Khác với mean và median, mode có thể áp dụng trực tiếp cho biến định tính, ví dụ giới tính, loại đau ngực, nhãn bệnh, vị trí tổn thương hoặc lớp văn bản.

Mode phù hợp nhất với biến định tính như \texttt{cp}, \texttt{sex}, \texttt{dx}, \texttt{Diagnosis}; cũng dùng được với biến định lượng rời rạc như tuổi hoặc số mạch máu. Trường hợp sử dụng cụ thể gồm xác định loại đau ngực phổ biến nhất trong Heart Disease, lớp tổn thương da xuất hiện nhiều nhất trong HAM10000, nhãn chẩn đoán chiếm ưu thế trong Breast Cancer hoặc mức đánh giá sức khỏe phổ biến trong CDC Diabetes. Trong tiền xử lý, mode thường được dùng để điền khuyết cho biến phân loại vì nó giữ lại nhóm phổ biến nhất. \textbf{Hạn chế:} một biến có thể có một mode, nhiều mode hoặc không có mode rõ ràng nếu tần suất quá phân tán. Mode không ổn định với biến liên tục có quá nhiều giá trị khác nhau, khi đó nên nhóm khoảng trước khi tính. Mode chỉ phản ánh giá trị phổ biến nhất, không cho biết trung tâm số học hay mức phân tán của dữ liệu.
...
Từ Bảng \ref{tab:chapter1_mode_example}, \texttt{cp=4} là loại đau ngực phổ biến nhất, \texttt{sex=1} là nhóm giới tính chiếm ưu thế, \texttt{oldpeak=0.0} cho thấy nhiều bệnh nhân không có ST depression trong bài test gắng sức, và \texttt{ca=0} là số mạch máu thường gặp nhất. Với \texttt{chol}, có nhiều mode cùng xuất hiện, minh họa trường hợp dữ liệu đa mode.

\subsection{Percentiles}

Percentile mô tả vị trí tương đối của một giá trị trong phân phối và thường được dùng để xây dựng quartiles, IQR và boxplot \cite{Sahay2021Essentials,DeMauro2021DataAnalytics}. Gọi $P_k$ là percentile thứ $k$, có thể hiểu $P_k$ là ngưỡng sao cho xấp xỉ $k\%$ quan sát có giá trị nhỏ hơn hoặc bằng ngưỡng đó. Với dãy đã sắp xếp tăng dần, vị trí percentile thường được xác định gần đúng bằng:

\begin{equation}
pos_k = \frac{k}{100}(n+1)
\end{equation}

Sau đó $P_k$ được lấy tại vị trí $pos_k$ hoặc nội suy giữa hai vị trí lân cận nếu $pos_k$ không nguyên. Trong đó $k$ là mức phần trăm cần tính, $n$ là số quan sát và $pos_k$ là vị trí lý thuyết trong dãy đã sắp xếp. Percentiles mở rộng ý tưởng median từ một điểm chia 50\% thành nhiều ngưỡng chia khác nhau trên toàn bộ phân phối. Nhờ đó, người phân tích không chỉ biết trung tâm dữ liệu mà còn biết vùng thấp, vùng cao và độ trải của dữ liệu.

Percentiles phù hợp khi cần mô tả phân bố theo ngưỡng, đánh giá nhóm thấp/trung bình/cao hoặc xác định vùng dữ liệu trung tâm. Percentile thứ 25, 50 và 75 lần lượt là $Q_1$, median và $Q_3$. Trong dữ liệu y tế, percentiles giúp xác định nhóm bệnh nhân có chỉ số quá thấp hoặc quá cao; trong chuỗi thời gian, percentiles giúp phân biệt ngày tiêu thụ bình thường và ngày tiêu thụ bất thường; trong dữ liệu ảnh, percentiles của độ sáng pixel giúp mô tả ảnh tối/sáng. Trường hợp sử dụng thực tế gồm đặt ngưỡng cảnh báo cholesterol ở nhóm 10\% cao nhất, chia bệnh nhân theo nhóm tuổi, xác định mức tiêu thụ điện bất thường trên $P_{95}$ hoặc cắt ngưỡng pixel khi chuẩn hóa ảnh. \textbf{Hạn chế:} percentile phụ thuộc vào cách nội suy và kích thước mẫu; với mẫu nhỏ, ngưỡng phân vị có thể không ổn định. Percentile cũng chỉ cho biết vị trí tương đối, không giải thích nguyên nhân vì sao một giá trị nằm ở vùng cao hoặc thấp.
...
\begin{tabular}{lrrrr}
\toprule
Đặc tính & $n$ & $P_{25}$ & $P_{50}$ & $P_{75}$ \\
\midrule
\texttt{age} -- tuổi & 303 & 48.00 & 56.00 & 61.00 \\
\texttt{trestbps} -- huyết áp nghỉ & 303 & 120.00 & 130.00 & 140.00 \\
\texttt{chol} -- cholesterol & 303 & 211.00 & 241.00 & 275.00 \\
\texttt{thalach} -- nhịp tim tối đa & 303 & 133.50 & 153.00 & 166.00 \\
\texttt{oldpeak} -- ST depression & 303 & 0.00 & 0.80 & 1.60 \\
\bottomrule
\end{tabular}
\end{table}
...
Mean và median thể hiện sự đánh đổi điển hình giữa hiệu quả và tính bền vững: mean tận dụng toàn bộ dữ liệu nhưng nhạy với outlier, còn median bền vững nhưng bỏ qua độ lớn của hầu hết quan sát. Trimmed mean (trung bình cắt ngọn) là lựa chọn trung gian được John W. Tukey giới thiệu trong công trình nền tảng về phân tích dữ liệu khám phá như một số đo trung tâm bền vững khi dữ liệu có vài giá trị cực trị nhưng vẫn muốn giữ ý nghĩa ``điểm cân bằng số học'' của mean \cite{Tukey1977EDA}. Với tỷ lệ cắt $\alpha$ ($0 \le \alpha < 0.5$), trimmed mean loại bỏ $\alpha n$ giá trị nhỏ nhất và $\alpha n$ giá trị lớn nhất sau khi sắp xếp rồi tính trung bình phần còn lại:

\begin{equation}
\bar{x}_{\alpha} = \frac{1}{n - 2\lfloor \alpha n \rfloor}\sum_{i=\lfloor \alpha n \rfloor+1}^{n-\lfloor \alpha n \rfloor} x_{(i)}
\end{equation}

Trong đó $x_{(1)} \le x_{(2)} \le \cdots \le x_{(n)}$ là dãy đã sắp xếp, $\lfloor \alpha n \rfloor$ là số quan sát bị cắt ở mỗi đầu, và $\bar{x}_{\alpha}$ là trung bình của phần giữ lại. Khi $\alpha = 0$, trimmed mean trở về mean thường; khi $\alpha$ tiệm cận 0.5, nó tiệm cận median. Việc chọn $\alpha$ thường vào khoảng 5\%--20\% tùy mức độ nghi ngờ outlier.

Trimmed mean phù hợp khi dữ liệu định lượng có phân phối hơi lệch hoặc vài giá trị cực trị, nhưng người phân tích vẫn muốn một số đo trung tâm dạng trung bình số học thay vì median thuần túy. Trường hợp sử dụng thực tế gồm báo cáo cholesterol đại diện bền vững cho Heart Disease khi có vài bệnh nhân có giá trị rất cao, tóm tắt mức tiêu thụ điện điển hình của chuỗi Appliances Energy loại trừ vài ngày đỉnh điểm, hoặc ước lượng độ sáng đại diện của ảnh khi có vùng quá sáng cục bộ. \textbf{Hạn chế:} trimmed mean loại bỏ thông tin ở hai đầu phân phối, nên không thích hợp khi các giá trị cực trị mang ý nghĩa thực tế cần giữ (ví dụ ngày tiêu thụ điện đỉnh là đối tượng quan tâm), và việc chọn $\alpha$ có phần chủ quan.
...
Geometric mean (trung bình nhân) là số đo trung tâm dành riêng cho biến dương, đặc biệt hữu ích khi dữ liệu có phạm vi giá trị rộng, phân phối lệch phải hoặc biến đổi theo cấp số nhân như tỷ lệ tăng trưởng, độ sáng pixel, mật độ năng lượng. Trung bình nhân được định nghĩa là căn bậc $n$ của tích các giá trị \cite{NIST2013Handbook}:

\begin{equation}
\bar{x}_G = \sqrt[n]{x_1 \cdot x_2 \cdots x_n} = \exp\!\left(\frac{1}{n}\sum_{i=1}^{n}\ln x_i\right)
\end{equation}

Trong đó $x_i > 0$ là quan sát thứ $i$, $n$ là số quan sát dương và $\bar{x}_G$ là trung bình nhân. Dạng $\exp\!\left(\frac{1}{n}\sum \ln x_i\right)$ tương đương arithmetic mean của logarit rồi lấy mũ, vừa tránh tràn số với tích lớn, vừa gợi nối trực tiếp với log transform dùng trong tiền xử lý (mục \ref{sec:logtransform}). Trung bình nhân luôn nhỏ hơn hoặc bằng mean số học khi mọi $x_i > 0$, và khoảng cách giữa hai số đo càng lớn thì phân phối lệch phải càng mạnh.

Geometric mean phù hợp khi biến luôn dương và cần ước lượng trung tâm trên thang multiplicative, ví dụ độ sáng ảnh (pixel intensity), mức tiêu thụ năng lượng, hoặc thời gian phản hồi. Trung bình nhân cũng là công cụ chuẩn để tổng hợp các tỷ lệ tăng trưởng (ví dụ tốc độ tăng số ca bệnh theo tuần) vì cộng log của các tỷ lệ đúng về mặt đại số. \textbf{Hạn chế:} trung bình nhân chỉ định nghĩa khi $x_i > 0$, nên không dùng trực tiếp cho biến có giá trị 0 hoặc âm; với \texttt{oldpeak} trong Heart Disease có tới 99/303 bệnh nhân mang giá trị 0, trung bình nhân không phải lựa chọn phù hợp. Khi cần dùng trên dữ liệu có chứa 0, thường phải dịch chuyển giá trị ($x_i + c$) trước khi tính, và lựa chọn $c$ sẽ ảnh hưởng đến kết quả.
...
Trong đó $x_{max}$ là giá trị lớn nhất và $x_{min}$ là giá trị nhỏ nhất của biến. Range là số đo đơn giản nhất để biết toàn bộ miền biến thiên của dữ liệu. Chỉ số này phù hợp khi cần kiểm tra nhanh dữ liệu có trải quá rộng hay có giá trị bất thường hay không. Trường hợp sử dụng gồm kiểm tra miền tuổi bệnh nhân, khoảng dao động cholesterol, biên độ tiêu thụ năng lượng trong ngày hoặc khoảng cường độ pixel của ảnh. \textbf{Hạn chế:} range chỉ dùng hai điểm cực trị nên rất nhạy với outlier và không phản ánh hình dạng phân phối bên trong. Hai tập dữ liệu có cùng range vẫn có thể có mức tập trung hoàn toàn khác nhau. Range cũng không cho biết phần lớn quan sát nằm gần min, gần max hay ở giữa.
...
\subsection{Variance và Standard Deviation}
...
Trong đó $x_i$ là quan sát thứ $i$, $\bar{x}$ là mean, $n$ là số quan sát và $n-1$ là bậc tự do khi tính phương sai mẫu. Mỗi sai lệch $(x_i-\bar{x})$ được bình phương để loại dấu âm và làm nổi bật các quan sát nằm xa trung tâm. Phương sai và độ lệch chuẩn cùng đo mức độ phân tán, nhưng khác nhau ở đơn vị và cách diễn giải. Phương sai $s^2$ lấy trung bình bình phương khoảng cách từ từng quan sát đến mean, nên đơn vị của nó là bình phương đơn vị gốc. Ví dụ nếu \texttt{chol} đo bằng mg/dL thì phương sai có đơn vị $(mg/dL)^2$, khó diễn giải trực tiếp trong ngữ cảnh y tế. Độ lệch chuẩn $s$ là căn bậc hai của phương sai, nên quay về cùng đơn vị với biến gốc và dễ đọc hơn: ``giá trị thường lệch khỏi mean khoảng bao nhiêu đơn vị''.

Về vai trò phân tích, phương sai hữu ích trong công thức toán học, tối ưu hóa mô hình, phân tích phương sai và các thuật toán học máy; độ lệch chuẩn hữu ích hơn khi trình bày báo cáo, so sánh mức dao động và giải thích cho người đọc. Trường hợp sử dụng gồm so sánh độ dao động huyết áp giữa các nhóm bệnh, đánh giá biến nào cần chuẩn hóa trước SVM/logistic regression, đo mức ổn định của năng lượng tiêu thụ hoặc mô tả độ phân tán đặc trưng ảnh. \textbf{Hạn chế:} phương sai nhấn mạnh mạnh hơn các sai lệch lớn vì bình phương khoảng cách, do đó rất nhạy với outlier. Độ lệch chuẩn cũng bị ảnh hưởng bởi outlier, nhưng dễ diễn giải hơn vì cùng đơn vị với dữ liệu. Cả hai chỉ số đều nên được đọc kèm histogram hoặc boxplot vì cùng một standard deviation có thể đi kèm các hình dạng phân phối khác nhau.
...
Trong đó $s$ là độ lệch chuẩn và $\bar{x}$ là mean của biến. CV là số đo không có đơn vị vì đã chuẩn hóa độ lệch chuẩn theo mean. Nhờ vậy, CV hữu ích khi cần so sánh mức biến động giữa các biến có thang đo hoặc đơn vị đo khác nhau, ví dụ tuổi, cholesterol, huyết áp, năng lượng tiêu thụ và độ sáng ảnh. CV càng lớn thì biến càng dao động mạnh tương đối với mức trung bình của chính nó.

CV phù hợp khi $\bar{x}$ dương và đủ xa 0. Trường hợp sử dụng gồm so sánh độ biến động tương đối giữa tuổi và cholesterol, giữa các cảm biến nhiệt độ/độ ẩm trong Appliances, hoặc giữa các đặc trưng ảnh có thang đo khác nhau. \textbf{Hạn chế:} nếu mean gần 0 hoặc có thể âm, CV dễ gây hiểu sai vì mẫu số quá nhỏ hoặc đổi dấu. CV cũng nên được đọc cùng histogram/boxplot vì một biến có CV cao có thể do outlier chứ không phải do biến động tự nhiên. CV không phù hợp cho thang đo có điểm 0 tùy ý hoặc biến đã chuẩn hóa quanh 0.
...
Quy tắc phát hiện outlier phổ biến:
...
Trong đó $Q_1=P_{25}$ là tứ phân vị dưới và $Q_3=P_{75}$ là tứ phân vị trên. IQR ít bị ảnh hưởng bởi outlier hơn range vì nó bỏ qua 25\% thấp nhất và 25\% cao nhất, chỉ tập trung vào vùng dữ liệu trung tâm. Do đó, IQR phù hợp khi dữ liệu lệch, có ngoại lệ hoặc cần so sánh độ phân tán bền vững giữa các nhóm. Trường hợp sử dụng gồm phát hiện cholesterol bất thường, so sánh vùng dao động BMI giữa nhóm bệnh/không bệnh, xác định ngày tiêu thụ điện ngoài mức thông thường hoặc kiểm tra ảnh có độ sáng lệch nhiều so với phần lớn mẫu.

Giá trị nhỏ hơn \texttt{Lower} hoặc lớn hơn \texttt{Upper} được xem là ứng viên outlier. \textbf{Hạn chế:} quy tắc $1.5\times IQR$ không khẳng định chắc chắn một điểm là lỗi dữ liệu; nó chỉ đánh dấu điểm cần kiểm tra thêm bằng kiến thức miền. IQR bỏ qua 50\% dữ liệu ngoài vùng giữa nên không mô tả đầy đủ phần đuôi phân phối. Trong dữ liệu y tế, một giá trị cholesterol rất cao có thể là lỗi nhập liệu, nhưng cũng có thể là bệnh nhân thật sự có chỉ số bất thường.

\textbf{Ví dụ.} Bảng \ref{tab:chapter1_iqr_example} minh họa IQR trên nhiều đặc tính. Với \texttt{chol}, $Q_1=211$, $Q_3=275$, nên $IQR=64$; biên trên là $275+1.5\times64=371$. Vì max thô là 564, giá trị này vượt biên trên và được xem là ứng viên outlier.
...
Standard deviation là số đo phân tán phổ biến nhất nhưng, vì bình phương sai lệch $(x_i-\bar{x})^2$, nó rất nhạy với outlier: chỉ một vài giá trị cực trị cũng làm $s$ phồng lên đáng kể. Mean Absolute Deviation (MAD) -- độ lệch tuyệt đối trung bình -- là số đo phân tán đơn giản hơn, lấy trung bình của các khoảng cách tuyệt đối đến mean thay vì bình phương chúng. Công thức MAD (ký hiệu $D_{\text{mean}}$ để phân biệt với median-MAD ở mục kế tiếp) là:

\begin{equation}
D_{\text{mean}} = \frac{1}{n}\sum_{i=1}^{n}\lvert x_i - \bar{x}\rvert
\end{equation}

Trong đó $x_i$ là quan sát thứ $i$, $\bar{x}$ là mean, $n$ là số quan sát và $\lvert \cdot \rvert$ là giá trị tuyệt đối. Việc dùng trị tuyệt đối thay vì bình phương làm MAD ít bị kéo phồng bởi các sai lệch lớn hơn so với standard deviation, đồng thời giữ cùng đơn vị với dữ liệu gốc nên dễ diễn giải: ``giá trị thường lệch khỏi mean khoảng bao nhiêu đơn vị''. Dưới phân phối chuẩn, MAD xấp xỉ $0.8s$ (cụ thể $\sqrt{2/\pi}\,s \approx 0.7979s$).

$D_{\text{mean}}$ phù hợp khi cần một số đo phân tán dễ giải thích, ít nhạy với một vài cực trị hơn standard deviation, nhưng vẫn gắn với mean để giữ tính liên tục với các phân tích dùng mean. Trường hợp sử dụng thực tế gồm tóm tắt mức dao động cholesterol hoặc huyết áp khi có vài bệnh nhân ngoại lệ, mô tả độ ổn định tiêu thụ điện của Appliances loại trừ các ngày đỉnh, hoặc đo độ phân tán cường độ pixel ảnh. \textbf{Hạn chế:} vì vẫn tham chiếu đến mean (bản thân mean đã bị outlier kéo lệch), $D_{\text{mean}}$ chỉ giảm bớt chứ không triệt tiêu hoàn toàn ảnh hưởng của cực trị; khi cần độ bền vững cao hơn, nên dùng median-MAD ở mục kế tiếp.
...
\subsection{Median Absolute Deviation (MAD bền vững)}
...
Trong đó $\tilde{x}$ là median của mẫu và $\text{median}(\cdot)$ lấy giá trị ở giữa của các độ lệch tuyệt đối. Để so sánh trực tiếp với standard deviation dưới phân phối chuẩn, người ta thường nhân MAD với hệ số 1.4826:
...
$\hat{\sigma}_{\text{MAD}}$ phù hợp khi dữ liệu có nhiều outlier, phân phối lệch nặng hoặc cần một chuẩn tham chiếu bền vững để đối sánh với standard deviation. Trường hợp sử dụng thực tế gồm ước lượng độ phân tán ``thực sự'' của cholesterol khi bác bỏ các giá trị cực cao, mô tả độ dao động của tín hiệu cảm biến HAR/MHEALTH có nhiễu đột biến, hoặc so sánh mức phân tán bền vững giữa các đặc trưng ảnh trước khi chuẩn hóa. \textbf{Hạn chế:} hiệu quả thống kê (Gaussian efficiency) của MAD chỉ khoảng 37\%, thấp hơn nhiều so với standard deviation; vì vậy MAD tốt cho khâu phát hiện/nhận diện outlier nhưng khi dữ liệu đã sạch, standard deviation vẫn là lựa chọn hiệu quả hơn. MAD cũng có ngưỡng làm tròn: với mẫu rất nhỏ hoặc dữ liệu rời rạc ít giá trị, các độ lệch tuyệt đối có thể trùng nhau làm MAD kém sắc nét.

\textbf{Ví dụ.} Bảng \ref{tab:chapter1_mad_median_example} so sánh ba số đo phân tán trên các đặc tính Heart Disease. Với \texttt{chol}, $s=51.78$ và $\hat{\sigma}_{\text{MAD}}=47.44$ -- hai giá trị khá gần nhau, cho thấy phần lớn dữ liệu cholesterol khá ổn định và các cực trị chỉ đẩy $s$ lên một chút. Trong khi đó, với \texttt{thalach}, $\hat{\sigma}_{\text{MAD}}=22.24$ gần như bằng $s=22.88$, phù hợp với phân phối ít lệch của nhịp tim tối đa. Sự so sánh giữa $s$ và $\hat{\sigma}_{\text{MAD}}$ là một cách nhanh gọn để đánh giá mức độ ảnh hưởng của outlier lên độ phân tán.
...
Trong đó $I(\cdot)$ là hàm chỉ báo, nhận giá trị 1 nếu điều kiện đúng và 0 nếu sai. Tần suất tương đối của lớp $c_j$ là:

\begin{equation}
p_j = \frac{f_j}{n}
\end{equation}

Bảng tần suất được dùng cho các biến như \texttt{sex}, \texttt{cp}, \texttt{Diagnosis}, \texttt{dx}, \texttt{localization}. Trường hợp sử dụng gồm kiểm tra tỷ lệ nam/nữ trong Heart Disease, phân bố loại đau ngực, tỷ lệ khối u lành/ác trong Breast Cancer, phân bố lớp tổn thương da trong HAM10000 và vị trí tổn thương phổ biến. Bảng \ref{tab:chapter1_freq_example} minh họa tần suất và tần suất tương đối của biến \texttt{cp}. \textbf{Hạn chế:} bảng tần suất chỉ mô tả số lượng/tỷ lệ từng lớp, không cho biết quan hệ giữa biến phân loại với các biến số khác. Nếu số lớp quá nhiều, bảng dài và khó đọc; khi đó cần gộp lớp hoặc dùng biểu đồ.
...
Giá trị $|z_i|$ lớn cho thấy quan sát nằm xa trung tâm dữ liệu. Trong nhiều bài toán, $|z_i| > 3$ được xem là dấu hiệu outlier. Z-score đặc biệt hữu ích khi cần so sánh các biến có đơn vị khác nhau, ví dụ tuổi, cholesterol và huyết áp. Trường hợp sử dụng gồm chuẩn hóa đặc trưng trước SVM/logistic regression, phát hiện bệnh nhân có chỉ số sinh học quá xa trung bình, chuẩn hóa cảm biến năng lượng và đưa các đặc trưng ảnh về cùng thang đo trước khi huấn luyện mô hình. \textbf{Hạn chế:} z-score phụ thuộc vào mean và standard deviation nên nhạy với outlier và kém phù hợp khi phân phối lệch mạnh. Ngưỡng $|z|>3$ chỉ là quy tắc kinh nghiệm, không thay thế kiểm tra bằng kiến thức miền.
...
\noindent\textbf{Phát hiện outlier theo IQR.} Dùng quy tắc râu boxplot với $Q_1$, $Q_3$, $IQR$ vừa tính:
\begin{enumerate}[leftmargin=2.2em,label=\textbf{\arabic*.}]
    \item Tính biên dưới:
    \begin{equation}
    ...
    \end{equation}
    \item So max thô với biên trên: $564 > 371$ nên max là ứng viên outlier. Đếm được 5/303 bệnh nhân ($\approx1.7\%$) có \texttt{chol} vượt 371.
\end{enumerate}

\noindent\textbf{Bảng tổng hợp và so sánh trước/sau xử lý outlier.} Bảng \ref{tab:chapter1_chol_detailed_stats} tổng hợp các giá trị vừa tính và đối chiếu với phiên bản đã cắt outlier ở ngưỡng 371.

\begin{table}[ht]
\centering
\caption[Minh họa thống kê mô tả biến \texttt{chol} Heart Disease]{Minh họa thống kê mô tả biến \texttt{chol} trong Heart Disease bằng cách tính thủ công từ dãy quan sát, kèm so sánh trước/sau khi cắt outlier ở ngưỡng $Upper=371$.}
...
Mean & 246.69 & 245.58 & Mean nhạy với outlier nên giảm nhẹ sau cắt. \\
...
\noindent\textbf{Bảng tổng hợp và so sánh trước/sau xử lý.} Bảng \ref{tab:chapter1_appliances_detailed_stats} tổng hợp các giá trị vừa tính và so sánh ba phiên bản: dữ liệu thô, dữ liệu sau khi cắt outlier ở ngưỡng 175 (theo quy tắc IQR) và dữ liệu sau khi biến đổi log1p. Việc đặt ba phiên bản cạnh nhau cho phép đối chiếu trực tiếp hai hướng xử lý và rút ra bài học về chuỗi thời gian.

\begin{table}[ht]
\centering
\caption[Minh họa thống kê mô tả biến \texttt{Appliances} Appliances Energy Prediction]{Minh họa thống kê mô tả biến \texttt{Appliances} trong Appliances Energy Prediction bằng cách tính thủ công từ dãy quan sát, kèm so sánh giữa dữ liệu thô, dữ liệu cắt outlier ở ngưỡng 175 và dữ liệu biến đổi log1p.}
...
\textbf{Nhận xét cho \texttt{Appliances}.} Bảng \ref{tab:chapter1_appliances_detailed_stats} cho thấy hai hướng xử lý dẫn tới hai kịch bản khác nhau. Nếu cắt outlier ở ngưỡng 175 như \texttt{chol}, mean giảm từ 97.70 xuống 78.89, $s$ giảm từ 102.52 xuống 42.96, nhưng \textbf{median, $IQR$ và $\hat{\sigma}_{\text{MAD}}$ gần như không đổi} -- đúng như lý thuyết về tính bền vững của nhóm số đo dựa trên median. Tuy nhiên, việc cắt bỏ 10.8\% mốc tiêu thụ đỉnh là \textit{mất thông tin dự báo quan trọng}, vì đây là các khung giờ tiêu thụ cao có thật chứ không phải lỗi. Nếu thay vào đó dùng log1p-transform (cột cuối), CV giảm từ 1.049 xuống 0.150, phân phối gần chuẩn hơn mà vẫn giữ toàn bộ mốc. Vì vậy, với chuỗi thời gian như \texttt{Appliances}, \textbf{log-transform là lựa chọn đúng} thay vì cắt outlier: nó nén đuôi phải, làm $s$ và CV giảm mạnh, nhưng vẫn bảo toàn ngày đỉnh cho mô hình dự báo. Khoảng cách lớn giữa $s=102.52$ và $\hat{\sigma}_{\text{MAD}}=29.65$ (hơn 3 lần) ở phiên bản thô chính là dấu hiệu để người phân tích nhận biết ngay rằng dữ liệu lệch phải nặng và cần biến đổi thay vì cắt.

\FloatBarrier
\subsection{So sánh hai biến và bài học phương pháp}

\begin{table}[ht]
\centering
\caption{So sánh các số đo thống kê giữa \texttt{chol} (nhóm A) và \texttt{Appliances} (nhóm B).}
\label{tab:chapter1_two_var_compare}
\small
\begin{tabular}{lrrp{0.36\textwidth}}
\toprule
Số đo & \texttt{chol} & \texttt{Appliances} & Bài đọc so sánh \\
\midrule
$n$ & 303 & 19\,735 & \texttt{Appliances} lớn hơn 65 lần, phân vị ổn định hơn. \\
$\bar{x}-Median$ & 5.69 & 37.70 & \texttt{Appliances} lệch phải mạnh hơn nhiều. \\
$CV$ & 0.210 & 1.049 & \texttt{chol} khá tập trung; \texttt{Appliances} cực kỳ phân tán. \\
$s$ vs $\hat{\sigma}_{\text{MAD}}$ & 51.78 vs 47.44 & 102.52 vs 29.65 & \texttt{chol}: gần nhau $\Rightarrow$ ít outlier; \texttt{Appliances}: cách xa $\Rightarrow$ đuôi phải nặng. \\
Tỷ lệ outlier IQR & 1.7\% & 10.8\% & \texttt{chol}: ứng viên lỗi cần xử lý; \texttt{Appliances}: đỉnh đích thực cần giữ. \\
Trimmed mean & 243.97 & 72.75 & Dùng cho cả hai khi mean bị kéo. \\
\bottomrule
\end{tabular}
\end{table}

Hai biến cho thấy cùng một bộ công thức thống kê nhưng dẫn tới những quyết định tiền xử lý trái ngược. Với \texttt{chol}, vì chỉ 1.7\% quan sát là outlier và chúng làm phồng $s$, cắt theo ngưỡng IQR là hợp lý; mean, $s$, variance thay đổi rõ sau cắt còn median, $IQR$, $\hat{\sigma}_{\text{MAD}}$ đứng yên -- minh chứng cho tính bền vững của nhóm số đo dựa trên median. Với \texttt{Appliances}, 10.8\% ``outlier'' theo công thức IQR thực ra là phần đuôi phân phối có thật, không được xóa mà cần biến đổi (log-transform) và giữ lại cho dự báo. Như vậy, không có số đo nào đúng tuyệt đối: mean/$s$ phù hợp khi dữ liệu sạch và cần hiệu quả; median/$IQR$/$\hat{\sigma}_{\text{MAD}}$ phù hợp khi cần bền vững; trimmed mean là cầu nối trung gian. Việc tính tay các bước trên hai biến thật -- từ sắp xếp dãy, thay số vào công thức đến đối chiếu trước/sau xử lý -- chính là cơ sở để ở Chương 3 chọn đúng số đo và biểu đồ cho từng biến trước khi vào học máy.

\section{Hệ thống biểu đồ trực quan hóa và tác dụng phân tích}

Hệ thống biểu đồ trong báo cáo được chọn theo ba tiêu chí: bản chất biến, câu hỏi phân tích và khả năng tái lập từ dữ liệu minh họa. Theo checklist của project, mỗi hình phải có đường dẫn dữ liệu, script sinh hình và diễn giải ngắn; vì vậy phần này không chỉ nêu tên biểu đồ mà còn chỉ rõ các bước vẽ, hình dạng/phân bố quan sát được, nhận xét dữ liệu, ý nghĩa phân tích và phạm vi áp dụng của từng biểu đồ. Các ví dụ được gắn với Heart Disease, Appliances Energy, HAM10000, Breast Cancer và CDC Diabetes. Để người đọc thấy rõ dữ liệu đầu vào của từng biểu đồ, mỗi nhóm biểu đồ chính được bổ sung một bảng dữ liệu mẫu rút gọn ngay trước hoặc sau hình minh họa.
...
\begin{enumerate}
    ...
    \item \textbf{Đọc hình dạng và phân bố:} xác định dữ liệu cân bằng hay lệch trái/lệch phải, một đỉnh hay nhiều đỉnh, phân tán rộng hay hẹp, có cụm, outlier hoặc mùa vụ hay không.
...
\midrule
Histogram lệch phải & Phần lớn giá trị nhỏ, ít giá trị rất lớn & Mean dễ bị kéo lên, mô hình nhạy outlier & Xem log-transform, median, IQR \\
Boxplot có nhiều điểm ngoài râu & Có ứng viên outlier & Cần kiểm tra lỗi nhập liệu hoặc giá trị hiếm hợp lệ & Winsorize/capping hoặc giữ nếu có ý nghĩa miền \\
...
\midrule
Histogram & Xem hình dáng phân phối, độ lệch, đa đỉnh & \texttt{Appliances}, \texttt{chol}, BMI & Phụ thuộc số bins, không chỉ rõ từng outlier \\
Boxplot & Phát hiện outlier theo IQR, so sánh nhóm & \texttt{chol} theo target, độ sáng ảnh theo lớp & Không mô tả rõ đa đỉnh, outlier chỉ là ứng viên \\
...
\subsubsection{Histogram: hình dáng phân phối}

Histogram chia miền giá trị thành các khoảng và đếm số quan sát trong từng khoảng. Biểu đồ này giúp nhận diện phân phối cân bằng, lệch trái, lệch phải, đa đỉnh hoặc có điểm bất thường. Trong báo cáo, histogram được dùng cho các biến như \texttt{age}, \texttt{chol}, \texttt{BMI}, \texttt{Appliances} và \texttt{age} trong HAM10000. Tác dụng chính là quyết định có cần log-transform, chuẩn hóa hoặc chia nhóm giá trị trước khi mô hình hóa hay không. \textbf{Hạn chế:} histogram phụ thuộc số bins; cùng một dữ liệu nhưng chọn bins khác nhau có thể làm hình dạng nhìn khác. Histogram cũng không hiển thị rõ từng điểm outlier như boxplot.
...
Bảng \ref{tab:chapter1_histogram_sample_data} minh họa dạng dữ liệu một biến dùng để tạo histogram. Trước khi vẽ, cần tính thủ công các khoảng lớp và tần suất. Ví dụ chọn ba khoảng $[50,100)$, $[100,200)$, $[200,300]$ cho sáu mẫu trên, ta có kết quả ở Bảng \ref{tab:chapter1_histogram_calc}.
...
Từ bảng tính này có thể vẽ tay histogram: trục hoành là khoảng giá trị, trục tung là tần suất. Cột log1p được tính bằng $\log(1+x)$, ví dụ $\log(1+60)=4.11$, nhằm chứng minh biến đổi log trước khi vẽ biểu đồ so sánh.

\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{../outputs/figures/appliances_energy_distribution_raw_vs_log.png}
\caption[Minh họa histogram Python]{Minh họa histogram raw và log1p trên Appliances Energy bằng Python. Python source: \texttt{src/analyze\_appliances\_energy.py}; figure path: \texttt{outputs/figures/appliances\_energy\_distribution\_raw\_vs\_log.png}.}
\label{fig:chapter1_histogram_example}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{../outputs/figures/excel_appliances_raw_vs_log_hist_preview.png}
\caption[Minh họa histogram Excel]{Minh họa histogram raw và log1p trên Appliances Energy bằng Excel. Excel source: \texttt{excel/excel\_visualization\_comparison.xlsx}; Excel preview: \texttt{outputs/figures/excel\_appliances\_raw\_vs\_log\_hist\_preview.png}.}
\label{fig:chapter1_histogram_example_excel}
\end{figure}

\textbf{Nhận xét và lý do chọn.} Histogram được chọn vì \texttt{Appliances} là biến định lượng liên tục và mục tiêu cần xem hình dạng phân phối. Hình \ref{fig:chapter1_histogram_example} cho thấy phân phối gốc lệch phải mạnh: phần lớn quan sát tập trung ở mức tiêu thụ thấp, nhưng vẫn có một số đỉnh tiêu thụ rất cao. Khi dùng log1p, phân phối gọn và cân bằng hơn. So sánh Python và Excel cho thấy hai công cụ cùng phản ánh một kết luận: dữ liệu năng lượng không nên đưa thẳng vào mô hình hồi quy mà cần xem xét biến đổi log hoặc xử lý outlier. Python phù hợp để tự động hóa và tái lập; Excel phù hợp để kiểm tra trực quan nhanh bằng bảng tính.

\subsubsection{Boxplot: phát hiện outliers dựa trên IQR}

Boxplot hiển thị median, $Q_1$, $Q_3$, IQR và các điểm ngoài ngưỡng $Q_1-1.5\times IQR$ hoặc $Q_3+1.5\times IQR$. Đây là biểu đồ quan trọng để phát hiện outlier và so sánh phân phối giữa các lớp. Ví dụ boxplot cholesterol theo \texttt{target\_binary} cho phép so sánh mức cholesterol giữa nhóm có và không có bệnh tim. Tác dụng phân tích là phát hiện biến cần xử lý ngoại lệ, đồng thời đánh giá biến định lượng có khả năng phân biệt nhãn hay không. \textbf{Hạn chế:} boxplot không mô tả rõ hình dạng phân phối bên trong; dữ liệu đa đỉnh có thể bị che khuất. Điểm ngoài râu hộp chỉ là ứng viên outlier, cần kiểm tra thêm bằng kiến thức miền.
...
5 & 0 & 354 & Giá trị cao \\
6 & 1 & 564 & Ứng viên outlier \\
\bottomrule
\end{tabular}
\end{table}

Bảng \ref{tab:chapter1_boxplot_sample_data} cho thấy dữ liệu boxplot cần một biến định lượng và một biến nhóm. Trước khi vẽ, cần tính các đại lượng hộp: $Q_1$, median, $Q_3$, IQR và biên outlier.
...
Ứng viên outlier & \texttt{chol} $>371$ hoặc $<115$ & 564 \\
\bottomrule
\end{tabular}
\end{table}

Từ các kết quả trên có thể vẽ tay boxplot: hộp kéo từ 211 đến 275, đường giữa ở 241, râu trên đến 371 và điểm 564 được vẽ riêng như outlier.

\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{../outputs/figures/heart_disease_cholesterol_boxplot.png}
\caption[Minh họa boxplot Heart Disease]{Minh họa boxplot: cholesterol theo nhóm bệnh tim. Python source: \texttt{src/analyze\_heart\_disease.py}; figure path: \texttt{outputs/figures/heart\_disease\_cholesterol\_boxplot.png}.}
\label{fig:chapter1_boxplot_example}
\end{figure}

\textbf{Nhận xét và lý do chọn.} Boxplot được chọn vì \texttt{chol} là biến định lượng liên tục và câu hỏi phân tích là ``có outlier hay không, phân phối hai nhóm target khác nhau thế nào?''. Hình \ref{fig:chapter1_boxplot_example} cho thấy cholesterol có các điểm nằm ngoài vùng râu hộp, tức là có ứng viên outlier theo quy tắc IQR. Median và IQR giúp đọc cấu trúc trung tâm tốt hơn mean khi dữ liệu có giá trị cực trị. So với histogram, boxplot không mô tả chi tiết hình dạng phân phối, nhưng tốt hơn trong việc so sánh hai nhóm và phát hiện điểm bất thường. Vì vậy, mẫu Heart Disease phù hợp với boxplot khi mục tiêu là đánh giá outlier và khả năng phân biệt nhóm bệnh/không bệnh.
...
Biểu đồ & Ý nghĩa & Trường hợp áp dụng & Hạn chế \\
\midrule
Scatter plot & Tìm liên hệ tuyến tính/phi tuyến, cụm, điểm bất thường & \texttt{age}--\texttt{thalach}, nhiệt độ--năng lượng & Rối khi nhiều điểm, không chứng minh nhân quả \\
Bar chart/Pie chart & Xem cơ cấu định tính, lớp phổ biến/hiếm & \texttt{cp}, \texttt{dx}, \texttt{Diagnosis} & Pie khó so sánh nhiều nhóm; bar không thể hiện quan hệ nếu không nhóm/chồng \\
Correlation heatmap & Xem tương quan nhiều biến định lượng & Chọn đặc trưng Heart Disease, Breast Cancer & Chủ yếu phản ánh tuyến tính, không chứng minh nhân quả \\
Pair/Hexbin plot & Xem nhiều cặp biến hoặc mật độ điểm dày & EDA nhiều biến định lượng & Dễ rối, tốn không gian báo cáo \\
Stacked bar/Mosaic & Xem quan hệ hai biến định tính & \texttt{dx} theo \texttt{localization}, target theo giới tính & Khó đọc khi nhiều nhóm nhỏ \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Scatter plot: tìm liên hệ tuyến tính/phi tuyến}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{../outputs/figures/heart_disease_age_thalach_scatter.png}
\caption[Minh họa scatter plot Python]{Minh họa scatter plot tuổi và nhịp tim tối đa trong Heart Disease bằng Python. Python source: \texttt{src/analyze\_heart\_disease.py}; figure path: \texttt{outputs/figures/heart\_disease\_age\_thalach\_scatter.png}.}
\label{fig:chapter1_scatter_example}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{../outputs/figures/excel_heart_age_thalach_scatter_preview.png}
\caption[Minh họa scatter plot Excel]{Minh họa scatter plot tuổi và nhịp tim tối đa trong Heart Disease bằng Excel. Excel source: \texttt{excel/excel\_visualization\_comparison.xlsx}; Excel preview: \texttt{outputs/figures/excel\_heart\_age\_thalach\_scatter\_preview.png}.}
...
\subsubsection{Bar chart/Pie chart: cơ cấu định tính}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{../outputs/figures/heart_disease_chest_pain_bar.png}
\caption[Minh họa bar chart Python]{Minh họa bar chart tần suất loại đau ngực Heart Disease bằng Python. Python source: \texttt{src/analyze\_heart\_disease.py}; figure path: \texttt{outputs/figures/heart\_disease\_chest\_pain\_bar.png}.}
\label{fig:chapter1_bar_example}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{../outputs/figures/excel_heart_cp_bar_preview.png}
\caption[Minh họa bar chart Excel]{Minh họa bar chart tần suất loại đau ngực Heart Disease bằng Excel. Excel source: \texttt{excel/excel\_visualization\_comparison.xlsx}; Excel preview: \texttt{outputs/figures/excel\_heart\_cp\_bar\_preview.png}.}
...
\subsubsection{Line chart cho chuỗi thời gian}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{../outputs/figures/appliances_energy_daily_line.png}
\caption[Minh họa line chart Python]{Minh họa line chart tiêu thụ năng lượng theo ngày trong Appliances Energy bằng Python. Python source: \texttt{src/analyze\_appliances\_energy.py}; figure path: \texttt{outputs/figures/appliances\_energy\_daily\_line.png}.}
\label{fig:chapter1_line_example}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{../outputs/figures/excel_appliances_daily_line_preview.png}
\caption[Minh họa line chart Excel]{Minh họa line chart tiêu thụ năng lượng theo ngày trong Appliances Energy bằng Excel. Excel source: \texttt{excel/excel\_visualization\_comparison.xlsx}; Excel preview: \texttt{outputs/figures/excel\_appliances\_daily\_line\_preview.png}.}
...
\subsubsection{Correlation heatmap: tương quan nhiều biến định lượng}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../outputs/figures/heart_disease_correlation_heatmap.png}
\caption[Minh họa correlation heatmap Heart Disease]{Minh họa correlation heatmap: tương quan biến trong Heart Disease. Python source: \texttt{src/analyze\_heart\_disease.py}; figure path: \texttt{outputs/figures/heart\_disease\_correlation\_heatmap.png}.}
...
\subsubsection{Hexbin plot cho dữ liệu hai biến dày điểm}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{../outputs/figures/ham10000_mean_intensity_boxplot.png}
\caption[Minh họa dữ liệu cho raincloud plot]{Cơ sở thay thế bằng raincloud plot: độ sáng ảnh HAM10000 theo lớp. Python source: \texttt{src/analyze\_ham10000.py}; figure path: \texttt{outputs/figures/ham10000\_mean\_intensity\_boxplot.png}.}
...
\centering
\includegraphics[width=0.90\textwidth]{../outputs/figures/heart_disease_age_thalach_scatter.png}
\caption[Minh họa dữ liệu cho hexbin plot]{Cơ sở thay thế bằng hexbin plot: tuổi và nhịp tim tối đa trong Heart Disease. Python source: \texttt{src/analyze\_heart\_disease.py}; figure path: \texttt{outputs/figures/heart\_disease\_age\_thalach\_scatter.png}.}
\label{fig:chapter1_extra_chart_examples_hexbin}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../outputs/figures/appliances_energy_monthly_ridgeline.png}
\caption[Minh họa ridgeline plot Appliances Energy]{Minh họa ridgeline plot: phân phối \texttt{log1p(Appliances)} theo tháng trong Appliances Energy. Python source: \texttt{src/analyze\_appliances\_energy.py}; figure path: \texttt{outputs/figures/appliances\_energy\_monthly\_ridgeline.png}.}
\label{fig:chapter1_ridgeline_example}
\end{figure}
...
Min-max scaling & Cần miền $[0,1]$, dữ liệu ảnh/neural network & Có outlier mạnh chưa xử lý & Độ sáng ảnh HAM10000 \\
...
Phân tích khám phá dữ liệu (Exploratory Data Analysis - EDA) là giai đoạn nối giữa dữ liệu thô và mô hình phân tích. Nếu bỏ qua EDA, người phân tích có thể đưa vào mô hình các biến bị lỗi kiểu dữ liệu, phân phối lệch quá mạnh, chứa outlier bất thường hoặc có target mất cân bằng. Trong báo cáo này, EDA được xem như một quy trình có kiểm soát gồm năm bước.
...
Điểm quan trọng là EDA không dừng ở việc tạo ra nhiều hình. Mỗi hình phải trả lời một câu hỏi cụ thể. Ví dụ, histogram trả lời câu hỏi ``biến này phân phối như thế nào?'', boxplot trả lời câu hỏi ``biến này có outlier không?'', scatter plot trả lời câu hỏi ``hai biến có quan hệ gì không?'', còn line chart trả lời câu hỏi ``dữ liệu thay đổi ra sao theo thời gian?''.
...
Trực quan hóa & Phân phối, outlier, quan hệ biến thế nào? & PNG figure, Excel chart \\
...
Có outlier không? & Boxplot & Dựa trên median, IQR và điểm ngoài ngưỡng \\
...
Trong báo cáo này, nguyên tắc chọn biểu đồ được áp dụng nhất quán. Với Heart Disease, histogram và boxplot được dùng để minh họa phân phối và outlier; scatter plot và heatmap được dùng để khảo sát quan hệ biến. Với Appliances Energy, line chart là bắt buộc vì dữ liệu có trục thời gian. Với HAM10000, bar chart được dùng cho phân phối lớp, còn boxplot được dùng cho đặc trưng độ sáng sau khi số hóa ảnh.
...
\begin{itemize}
    ...
    \item \textbf{Nhận xét tiền xử lý}: có cần cắt outlier, log-transform, chuẩn hóa hay không.
\end{itemize}
...
Bảng \ref{tab:chapter1_preprocess_compare} cho thấy tiền xử lý không làm thay đổi mạnh giá trị trung tâm, nhưng làm giảm độ biến động của các biến có outlier. Trường hợp rõ nhất là \texttt{chol}: standard deviation giảm từ 51.78 xuống 47.56, max giảm từ 564 xuống 371. Điều này phù hợp với mục tiêu của tiền xử lý: giữ lại cấu trúc chính của dữ liệu nhưng hạn chế ảnh hưởng của điểm cực trị.

\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{../outputs/figures/heart_disease_age_raw_vs_cleaned.png}
\caption[Ví dụ histogram tuổi trên Heart Disease]{Ví dụ histogram tuổi trước/sau xử lý trên Heart Disease. Python source: \texttt{src/analyze\_heart\_disease.py}; figure path: \texttt{outputs/figures/heart\_disease\_age\_raw\_vs\_cleaned.png}.}
...
\centering
\includegraphics[width=0.90\textwidth]{../outputs/figures/heart_disease_cholesterol_boxplot.png}
\caption[Ví dụ boxplot cholesterol trên Heart Disease]{Ví dụ boxplot cholesterol theo nhãn bệnh trên Heart Disease. Python source: \texttt{src/analyze\_heart\_disease.py}; figure path: \texttt{outputs/figures/heart\_disease\_cholesterol\_boxplot.png}.}
\label{fig:heart_chol_boxplot}
\end{figure}

\textbf{Lý do chọn biểu đồ.} Histogram được dùng vì biến \texttt{age} là biến định lượng, cần quan sát hình dạng phân phối trước và sau xử lý. Boxplot được dùng vì \texttt{chol} dễ có outlier và cần so sánh theo nhãn bệnh.
...
\centering
\includegraphics[width=0.90\textwidth]{../outputs/figures/heart_disease_age_thalach_scatter.png}
\caption[Ví dụ scatter plot trên Heart Disease]{Ví dụ scatter plot age và thalach trên Heart Disease. Python source: \texttt{src/analyze\_heart\_disease.py}; figure path: \texttt{outputs/figures/heart\_disease\_age\_thalach\_scatter.png}.}
...
\centering
\includegraphics[width=0.95\textwidth]{../outputs/figures/heart_disease_correlation_heatmap.png}
\caption[Ví dụ correlation heatmap trên Heart Disease]{Ví dụ correlation heatmap trên Heart Disease. Python source: \texttt{src/analyze\_heart\_disease.py}; figure path: \texttt{outputs/figures/heart\_disease\_correlation\_heatmap.png}.}
